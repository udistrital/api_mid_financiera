		...
		if err = request.SendJson(urlcrud+"/giro/RegistrarGiro", "POST", &response, giro); err == nil {
			if strings.Compare(response["Type"].(string), "success") == 0 {
				for _, element := range response["OrdenesPago"].([]interface{}) {
					err = request.GetJson(urlcrud+"/giro/GetCuentasEspeciales?idordenpago="+strconv.FormatFloat(element.(map[string]interface{})["Id"].(float64), 'f', -1, 64), &idCuentasEspeciales)
					for _, cuenta := range idCuentasEspeciales {
						if err = request.GetJson("http://"+beego.AppConfig.String("AdministrativaAmazonService")+"informacion_proveedor/?query=Id:"+cuenta.(map[string]interface{})["informacion_persona_juridica"].(string)+"&limit=1", &resProveedor); err == nil {
							if err = request.SendJson(urlcrud+"/giro/RegistrarGiroDescuentos?idcuenta="+cuenta.(map[string]interface{})["cuenta_especial"].(string)+"&idordenpago="+element.(map[string]interface{})["Id"].(string)+"&idgiro="+response["idNewGiro"].(string), "POST", &response, &resProveedor); err == nil {
