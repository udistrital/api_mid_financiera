...
func getNecesidadDesdeRp(registroPresupuestal interface{}) (outputNecesidad interface{}) {
	...
	if rowRp, e := registroPresupuestal.(map[string]interface{}); e {
		...
		beego.Info("http://" + beego.AppConfig.String("argoService") + "solicitud_rp/?query=Id:" + strconv.Itoa(int(rowRp["Solicitud"].(float64))) + "&limit:1")
...
func getAreaDeNecesidad(necesidad interface{}) (outputAreaNecesidad interface{}) {
	...
	if err := getJson("http://"+beego.AppConfig.String("argoService")+"dependencia_necesidad/?query=Necesidad.Id:"+strconv.Itoa(int(necesidad.(map[string]interface{})["Id"].(float64)))+"&limit:1", &dependenciaNecesidad); err == nil && dependenciaNecesidad != nil {
